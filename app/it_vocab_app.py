...
def _it_check_fn(guess, correct, direction, card):
    ...
    if direction == "word→en":
        options = [_strip_the(_it_normalize(o)) for o in re.split(r"[/,]", correct) if o.strip()]
        if not options:
            options = [c_norm]
        for opt in options:
            if g_norm == opt:
            ...
            if opt_w and guess_w and opt_w[0] == guess_w[0]:
                return "correct"
        for opt in options:
            if _it_is_close(g_norm, opt):
                return "correct"
            opt_w   = [w for w in opt.split()    if len(w) > 2]
            guess_w = [w for w in g_norm.split() if len(w) > 2]
            if opt_w and guess_w and _it_is_close(guess_w[0], opt_w[0]):
                return "correct"
        return "wrong"
    ...
    if g_bare == c_norm:
        expected = _it_normalize(resolve_expected_article(IT_LANG, card))
        if not expected:
